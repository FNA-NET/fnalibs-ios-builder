...
if [ ! -d "./FNA3D/" ]; then
	...
	git clone https://github.com/FNA-NET/FNA3D.git --recursive
...
if [ ! -d "./FAudio/" ]; then
	...
	git clone https://github.com/FNA-NET/FAudio.git
...
# Check for updates...
echo "Updating SDL2..."
cd SDL2 && git checkout release-2.0.16 && cd ..
...
git reset --hard
# Apply patch
git apply ../sdl2_fna_net_ios.patch
cd ..
